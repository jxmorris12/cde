...
from .embedding import DynamicNTKRotaryEmbedding, VarLengthRotaryEmbedding
...
class FlashAttention(nn.Module):
    ...
    def forward(
        ...
        if self.rotary_start_pos > 0:
            ...
            assert len(qkv.shape) == 4 # (b, s, n_head, dim)
            # full_seq_len = qkv.shape[0]
            start_seq_idxs = (cu_seqlens[:-1,None] + torch.arange(self.rotary_start_pos, device=cu_seqlens.device)[None]).flatten()
            no_rotary_token_mask = torch.zeros(qkv.shape[0], dtype=bool, device=start_seq_idxs.device)
            no_rotary_token_mask[start_seq_idxs] = 1
        ...
        # print("[3] post-rotary qkv:", qkv.norm(p=2))
        if self.rotary_start_pos > 0:
            ############## SECOND NEW PART ##############
