...
class RedditSampler(torch.utils.data.Sampler):
    ...
    def __init__(
            self, 
            dataset: RedditDataset, batch_size: int, 
        ...
        num_questions = { k: len_minus_batch(v) for k,v in self.dataset.subreddit_questions.items() }
    ...
    def __iter__(self) -> Iterable[Dict]:
        ...
        for i, subreddit_idx in enumerate(self.subreddit_batches):
            ...
            if pointer == 0:
                self.shuffle_func(self.dataset.subreddit_questions[subreddit_idx])
            subreddit_idx_pointer[subreddit_idx] += 1
            questions = self.dataset.subreddit_questions[subreddit_idx]
