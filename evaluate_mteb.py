...
ARGS_STR_DICT = {
    ...
    # so i think that's allowed in this case. strangely, the eval metrics started
    # going down after a while, but i'm not going to read into that too much until
    # we have the mteb numbers.
...
def main():
    ...
    for task_idx, task in enumerate(TASK_LIST_RETRIEVAL):
        ...
        evaluation.run(mteb_encoder, output_folder=os.path.join("results_mteb", args.model_key), batch_size=1024, corpus_chunk_size=500_000)
